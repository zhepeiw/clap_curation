...
def prepare_urbansound8k_csv(
    ...
    for idx, row in tqdm(meta_df.iterrows()):
        ...
        if fold in train_folds:
            train_data.append((fname, class_id, class_name))
        if fold in valid_folds:
            valid_data.append((fname, class_id, class_name))
        if fold in test_folds:
            test_data.append((fname, class_id, class_name))
    # training csv for each task
    for split, split_data in zip(['train', 'valid', 'test'], [train_data, valid_data, test_data]):
        data = [{
            'ID': i,
            'wav_path': e[0],
            'class_idx': e[1],
            'class_name': e[2].replace('_', ' ')
