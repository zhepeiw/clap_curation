...
def prepare_tauuas2019_csv(
    root_dir,
    output_dir,
):
    '''
        preparing csv files for training, validation, and test
    '''
    ...
    for split, meta_df in zip(['train', 'valid', 'test'], [train_meta, valid_meta, test_meta]):
        temp_data = []
        for idx, row in tqdm(meta_df.iterrows()):
            ...
            temp_data.append((fname, class_name))
        data = [{
            'ID': i,
            'wav_path': e[0],
            'class_name': e[1].replace('_', ' ')
        } for i, e in enumerate(temp_data)]
        df = pd.DataFrame(data)
        task_path = os.path.join(
            output_dir,
            '{}_raw.csv'.format(split)
        )
...
def prepare_tauuas2019_label_encoder(
    ...
    train_meta = pd.read_csv(train_csv, sep='\t')
    class_names = sorted(train_meta['scene_label'].unique().tolist())
    class_names = [e.replace('_', ' ') for e in class_names]
