...
class CLAP(sb.core.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        if stage == sb.Stage.VALID and self.hparams.use_zs_loader:
            ...
            loss = self.hparams.compute_cross_sim_cost(logits, targets)
            ...
            if self.hparams.global_contrastive:
                ...
                target_txt = torch.arange(similarity.shape[1]).long().to(self.device)
                target_aud = torch.arange(similarity.shape[0]).long().to(self.device)
                cross_loss_1 = self.hparams.compute_cross_sim_cost(similarity, target_txt)
                cross_loss_2 = self.hparams.compute_cross_sim_cost(similarity.T, target_aud)
                ...
                for i in range(n_gpus):
                    ...
                    target_txt = torch.arange(sim.shape[1]).long().to(self.device)
                    target_aud = torch.arange(sim.shape[0]).long().to(self.device)
                    cross_loss_1 += self.hparams.compute_cross_sim_cost(sim, target_txt)
                    cross_loss_2 += self.hparams.compute_cross_sim_cost(sim.T, target_aud)

            loss = 0.5 * (cross_loss_1 + cross_loss_2)
